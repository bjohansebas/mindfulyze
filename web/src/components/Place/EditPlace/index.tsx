...
import { getPlace, putPlace, type ResponsePlace } from 'services/place'
		...
		>
			<Box sx={{ width: '100%' }}>
				<TextField
					...
					value={newTextPlace}
					onChange={(e) => {
						setNewTextPlace(e.target.value)
					}}
				/>
			</Box>
			<Box
				...
				<Box sx={{ display: 'flex', width: '100%', justifyContent: 'center', gap: 2 }}>
					...
					<Button
						...
						disabled={
							loading ||
							place == null ||
							(newColor.slice(1) === place.color.code && newTextPlace === place.name) ||
							newTextPlace.length < 2
						}
					>
