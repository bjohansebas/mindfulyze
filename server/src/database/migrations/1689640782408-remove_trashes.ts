...
export class RemoveTrashes1689640782408 implements MigrationInterface {
  name = 'RemoveTrashes1689640782408';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "trash_thinks" DROP CONSTRAINT "fk_user_ts"`,
    );
    await queryRunner.query(
      `ALTER TABLE "trash_thinks" DROP CONSTRAINT "fk_place_ts"`,
    );
    await queryRunner.query(
      `ALTER TABLE "think_trash_emotions" DROP CONSTRAINT "fk_emotion_thiemo_trash"`,
    );
    await queryRunner.query(
      `ALTER TABLE "think_trash_emotions" DROP CONSTRAINT "fk_think_trash_thiemo"`,
    );
